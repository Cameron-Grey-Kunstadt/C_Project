...
This file will document some of the important things that I learned with regards to developing software in C. I will be making a demo of a basic Breakout game in order to learn some of the standard methods of programming a Windows application in C. This motivation for this is to have an example project of C to show in interviews and on github, and to start learning how to develop applications for Windows using only C. The file Breakout_V1 contains strictly the code I created following Dan Zaidan's breakout tutorial, up through the second episode, where a very standard breakout game was about finished. No code was copied, every line was written by hand, just following his instructions. The second file, Breakout_V2, seeks to create the same game, but to painstakingly document the process, and everything that I learned while going through it. Included are code files on how how to do various things that are important for making the larger game such as opening a window and creating a bitmap. There is also a Learning_Process.md file that goes through the steps taken to create the game in detail.
